...
Byte_buffer::Byte_buffer(Bytes const& bv) : byte_buf_(bv) {}


Byte_buffer::Byte_buffer(Hex_string const& str)
{
	initialise();
	size_t buf_size = str.size()/2;
	byte_buf_.resize(buf_size);
	std::string bstr;
	for (int i = 0; i <buf_size; ++i)
	{
		bstr = str.hex_string().substr(2 * i, 2);
		byte_buf_[i] = static_cast<Byte>(stoul(bstr,nullptr,16));
	}
}

Byte_buffer::Byte_buffer(const Byte* buf, size_t len)
{
	byte_buf_.assign(len, 0);
	for (int i = 0; i < len; ++i)
		byte_buf_[i] = buf[i];
}
...
{
	while (byte_buf_.size()>0 && byte_buf_.back() == 0)
		byte_buf_.pop_back();
}
...
std::vector<Byte_buffer> deserialise_byte_buffers(Byte_buffer const& bb)
{
	...
	size_t n_buffers=256*bb[0]+bb[1];
	if (n_buffers>std::numeric_limits<uint16_t>::max())
	...
	for (int i=0;i<n_buffers;++i)
	{
		size_t sz=bb[current_index]*256+bb[current_index+1];
		if (sz>std::numeric_limits<uint16_t>::max())
