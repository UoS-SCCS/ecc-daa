...
/* Move b to a if d=1 */
void FP256BN::FP2_cmove(FP2 *f,FP2 *g,int d)
{
    FP_cmove(&(f->a),&(g->a),d);
    FP_cmove(&(f->b),&(g->b),d);
}
...
/* SU= 16 */
int FP256BN::FP2_equals(FP2 *x,FP2 *y)
{
...
/* SU= 16 */
void FP256BN::FP2_from_FPs(FP2 *w,FP *x,FP *y)
{
    FP_copy(&(w->a),x);
    FP_copy(&(w->b),y);
}
...
/* SU= 16 */
void FP256BN::FP2_from_BIGs(FP2 *w,BIG x,BIG y)
{
    FP_nres(&(w->a),x);
    FP_nres(&(w->b),y);
}
...
/* Create FP2 from FP */
/* SU= 8 */
void FP256BN::FP2_from_FP(FP2 *w,FP *x)
{
    FP_copy(&(w->a),x);
    FP_zero(&(w->b));
}
...
/* Create FP2 from BIG */
/* SU= 8 */
void FP256BN::FP2_from_BIG(FP2 *w,BIG x)
{
    FP_nres(&(w->a),x);
    FP_zero(&(w->b));
}
...
/* FP2 copy w=x */
/* SU= 16 */
void FP256BN::FP2_copy(FP2 *w,FP2 *x)
{
    if (w==x) return;
    FP_copy(&(w->a),&(x->a));
    FP_copy(&(w->b),&(x->b));
...
/* Set w=-x */
/* SU= 88 */
void FP256BN::FP2_neg(FP2 *w,FP2 *x)
{
    /* Just one neg! */
    FP m,t;
//    FP2_norm(x);
...
/* Set w=conj(x) */
/* SU= 16 */
void FP256BN::FP2_conj(FP2 *w,FP2 *x)
{
    FP_copy(&(w->a),&(x->a));
	//FP_norm(&(x->b));
    FP_neg(&(w->b),&(x->b));
...
/* Set w=x+y */
/* SU= 16 */
void FP256BN::FP2_add(FP2 *w,FP2 *x,FP2 *y)
{
    FP_add(&(w->a),&(x->a),&(y->a));
    FP_add(&(w->b),&(x->b),&(y->b));
}
...
void FP256BN::FP2_sub(FP2 *w,FP2 *x,FP2 *y)
{
    FP2 m;
    FP2_neg(&m,y);
    FP2_add(w,x,&m);
...
/* SU= 16 */
void FP256BN::FP2_pmul(FP2 *w,FP2 *x,FP *s)
{
    FP_mul(&(w->a),&(x->a),s);
    FP_mul(&(w->b),&(x->b),s);
}
...
void FP256BN::FP2_imul(FP2 *w,FP2 *x,int s)
{
    FP_imul(&(w->a),&(x->a),s);
    FP_imul(&(w->b),&(x->b),s);
}
...
/* Set w=x^2 */
/* SU= 128 */
void FP256BN::FP2_sqr(FP2 *w,FP2 *x)
{
    FP w1,w3,mb;
...
void FP256BN::FP2_mul(FP2 *w,FP2 *x,FP2 *y)
{
	DBIG A,B,E,F,pR;
	BIG C,D,p;
...
/* SU= 8 */
void FP256BN::FP2_rawoutput(FP2 *w)
{
    printf("[");
    BIG_rawoutput(w->a.g);
    printf(",");
...
/* Set w=1/x */
/* SU= 128 */
void FP256BN::FP2_inv(FP2 *w,FP2 *x)
{
    BIG b;
	FP w1,w2;
...
/* Set w=x/2 */
/* SU= 16 */
void FP256BN::FP2_div2(FP2 *w,FP2 *x)
{
    FP_div2(&(w->a),&(x->a));
    FP_div2(&(w->b),&(x->b));
}
...
void FP256BN::FP2_div_ip2(FP2 *w)
{
    FP2 t;
	FP2_norm(&t);
    FP_add(&(t.a),&(w->a),&(w->b));
...
/* Set w=a^b mod m */
/* SU= 208 */
void FP256BN::FP2_pow(FP2 *r,FP2* a,BIG b)
{
    FP2 w;
	FP one;
    BIG z,zilch;
...
int FP256BN::FP2_sqrt(FP2 *w,FP2 *u)
{
    BIG b;
	FP w1,w2;
    FP2_copy(w,u);
...
/* Input MUST be normed */
void FP256BN::FP2_times_i(FP2 *w)
{
    FP z;
