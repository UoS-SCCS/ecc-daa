...
/* So when multiplying two numbers, their product *must* be less than MODBITS_FP256BN+BASEBITS_B256_56*NLEN_B256_56 */
...
void FP256BN::FP_mod(BIG r,DBIG d)
{
    ...
    if (MConst < NEXCESS_B256_56)
    {
		...
		BIG_norm(r);
        tw=r[NLEN_B256_56-1];
        r[NLEN_B256_56-1]&=TMASK_FP256BN;
    ...
    else
    {
        ...
        r[NLEN_B256_56-1]&=TMASK_FP256BN;
#if CHUNK == 16
        r[1]+=muladd(MConst,((tw>>TBITS_FP256BN)+(v<<(BASEBITS_B256_56-TBITS_FP256BN))),0,&r[0]);
#else
        r[0]+=MConst*((tw>>TBITS_FP256BN)+(v<<(BASEBITS_B256_56-TBITS_FP256BN)));
#endif
    }
    BIG_norm(r);
}
...
// Now multiply t by MConst..(?) 
    ...
    carry=r[NLEN_B256_56-1]>>TBITS_FP256BN;
...
#endif
/*
    if (c<=NEXCESS_B256_56 && a->XES*c <= FEXCESS_FP256BN)
	{
        BIG_imul(r->g,a->g,c);
		r->XES=a->XES*c;
		FP_norm(r);
