...
int FP256BN::ECP_KEY_PAIR_GENERATE(csprng *RNG,octet* S,octet *W)
{
    BIG r,gx,gy,s;
    ECP G;
    int res=0;
...
#if CURVETYPE_FP256BN!=MONTGOMERY
    W->len=2*EFS_FP256BN+1;
    W->val[0]=4;
    BIG_toBytes(&(W->val[1]),gx);
    BIG_toBytes(&(W->val[EFS_FP256BN+1]),gy);
#else
    W->len=EFS_FP256BN+1;
    W->val[0]=2;
    BIG_toBytes(&(W->val[1]),gx);
...
/* Validate public key */
int FP256BN::ECP_PUBLIC_KEY_VALIDATE(octet *W)
{
    BIG q,r,wx,k;
    ECP WP;
    int valid,nb;
...
#if CURVETYPE_FP256BN!=MONTGOMERY
    BIG wy;
    BIG_fromBytes(wy,&(W->val[EFS_FP256BN+1]));
...
int FP256BN::ECP_SVDP_DH(octet *S,octet *WD,octet *Z)
{
    BIG r,s,wx;
    int valid;
    ECP W;
...
#if CURVETYPE_FP256BN!=MONTGOMERY
    BIG wy;
    BIG_fromBytes(wy,&(WD->val[EFS_FP256BN+1]));
...
int FP256BN::ECP_SP_DSA(int sha,csprng *RNG,octet *K,octet *S,octet *F,octet *C,octet *D)
{
    char h[128];
    ...
    int hlen=H.len;
    if (H.len>MODBYTES_B256_56) hlen=MODBYTES_B256_56;
...
int FP256BN::ECP_VP_DSA(int sha,octet *W,octet *F, octet *C,octet *D)
{
    char h[128];
    octet H= {0,sizeof(h),h};
    ...
    int hlen=H.len;
    if (hlen>MODBYTES_B256_56) hlen=MODBYTES_B256_56;
...
void FP256BN::ECP_ECIES_ENCRYPT(int sha,octet *P1,octet *P2,csprng *RNG,octet *W,octet *M,int tlen,octet *V,octet *C,octet *T)
{
    ...
    KDF2(sha,&VZ,P1,2*AESKEY_FP256BN,&K);
...
int FP256BN::ECP_ECIES_DECRYPT(int sha,octet *P1,octet *P2,octet *V,octet *C,octet *T,octet *U,octet *M)
{
