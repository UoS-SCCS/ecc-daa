...
static void FP256BN::mpin_hash(int sha,FP4 *f, ECP *P,octet *w)
{
    ...
    hash256 sha256;
    hash512 sha512;
    char t[6*MODBYTES_B256_56];  // to hold 6 BIGs
    int hlen=sha;
    ...
    BIG_toBytes(&t[0],x);
    FP_redc(x,&(f->a.b));
    BIG_toBytes(&t[MODBYTES_B256_56],x);
    FP_redc(x,&(f->b.a));
    BIG_toBytes(&t[2*MODBYTES_B256_56],x);
    FP_redc(x,&(f->b.b));
    BIG_toBytes(&t[3*MODBYTES_B256_56],x);
    ECP_get(x,y,P);
    BIG_toBytes(&t[4*MODBYTES_B256_56],x);
    BIG_toBytes(&t[5*MODBYTES_B256_56],y);
    ...
    case SHA256:
        HASH256_init(&sha256);
        for (i=0; i<6*MODBYTES_B256_56; i++) HASH256_process(&sha256,t[i]);
        HASH256_hash(&sha256,h);
        break;
    case SHA384:
        HASH384_init(&sha512);
        for (i=0; i<6*MODBYTES_B256_56; i++) HASH384_process(&sha512,t[i]);
        HASH384_hash(&sha512,h);
        break;
    case SHA512:
        HASH512_init(&sha512);
        for (i=0; i<6*MODBYTES_B256_56; i++) HASH512_process(&sha512,t[i]);
