...
/* a=1/a mod 2^BIGBITS_B1024_58. This is very fast! */
void B1024_58::BIG_invmod2m(BIG a)
{
    int i;
    BIG U,t1,b,c;
    BIG_zero(U);
    BIG_inc(U,invmod256(BIG_lastbits(a,8)));
    for (i=8; i<BIGBITS_B1024_58; i<<=1)
    {
		BIG_norm(U);
        BIG_copy(b,a);
...
/* shift right by BIGBITS_B1024_58-bit words */
...
/* shift left by BIGBITS_B1024_58-bit words */
...
void RSA2048::FF_init(BIG x[],sign32 m,int n)
{
    ...
    BIG_zero(x[0]);
#if CHUNK<64
    x[0][0]=(chunk)(m&BMASK_B1024_58);
    x[0][1]=(chunk)(m>>BASEBITS_B1024_58);
#else
    x[0][0]=(chunk)m;
#endif
    for (i=1; i<n; i++)
...
static void RSA2048::FF_rnorm(BIG z[],int zp,int n)
{
    ...
    for (i=0; i<n-1; i++)
    {
        ...
        z[zp+i][NLEN_B1024_58-1]^=carry<<P_TBITS_RSA2048; /* remove it */
        z[zp+i+1][0]+=carry;
    }
    carry=BIG_norm(z[zp+n-1]);
    if (trunc) z[zp+n-1][NLEN_B1024_58-1]^=carry<<P_TBITS_RSA2048;
}
...
void RSA2048::FF_shl(BIG x[],int n)
{
    ...
    for (i=0; i<n-1; i++)
    {
        ...
        x[i][0]|=delay_carry;
        x[i][NLEN_B1024_58-1]^=(chunk)carry<<P_TBITS_RSA2048;
        delay_carry=carry;
    }
    BIG_fshl(x[n-1],1);
    x[n-1][0]|=delay_carry;
...
void RSA2048::FF_shr(BIG x[],int n)
{
    ...
    for (i=n-1; i>0; i--)
    {
        ...
        x[i-1][NLEN_B1024_58-1]|=(chunk)carry<<P_TBITS_RSA2048;
    }
    BIG_fshr(x[0],1);
}
...
{
    int i;
    w->len=n*MODBYTES_B1024_58;
    for (i=0; i<n; i++)
    {
        BIG_toBytes(&(w->val[(n-i-1)*MODBYTES_B1024_58]),x[i]);
    }
}
...
void RSA2048::FF_fromOctet(BIG x[],octet *w,int n)
{
    ...
    for (i=0; i<n; i++)
    {
        BIG_fromBytes(x[i],&(w->val[(n-i-1)*MODBYTES_B1024_58]));
    }
}
...
static void RSA2048::FF_karmul(BIG z[],int zp,BIG x[],int xp,BIG y[],int yp,BIG t[],int tp,int n)
{
    ...
    if (n==1)
    {
        ...
        BIG_split(z[zp+1],z[zp],t[tp],BIGBITS_B1024_58);
        return;
    }
...
static void RSA2048::FF_nres(BIG a[],BIG m[],int n)
{
	...
	{
		BIG_dscopy(d[0],a[0]);
		BIG_dshl(d[0],NLEN_B1024_58*BASEBITS_B1024_58);
...
static void RSA2048::FF_redc(BIG a[],BIG m[],BIG ND[],int n)
{
	...
	if (n==1)
	{
		BIG_dzero(d[0]);
		BIG_dscopy(d[0],a[0]);
		BIG_monty(a[0],m[0],((chunk)1<<BASEBITS_B1024_58)-ND[0][0],d[0]);
	}
	else
	{
		FF_mod(a,m,n);
...
static void RSA2048::FF_invmod2m(BIG U[],BIG a[],int n)
{
    int i;
#ifndef C99
    BIG t1[FFLEN_RSA2048],b[FFLEN_RSA2048],c[FFLEN_RSA2048];
#else
    BIG t1[2*n],b[n],c[n];
#endif
...
void RSA2048::FF_random(BIG x[],csprng *rng,int n)
{
    ...
    while (BIG_nbits(x[n-1])<MODBYTES_B1024_58*8) BIG_random(x[n-1],rng);
}
...
static void RSA2048::FF_modmul(BIG z[],BIG x[],BIG y[],BIG p[],BIG ND[],int n)
{
	...
	{
		BIG_mul(d[0],x[0],y[0]);
		BIG_monty(z[0],p[0],((chunk)1<<BASEBITS_B1024_58)-ND[0][0],d[0]);
	}
	else
	{
		FF_mul(d,x,y,n);
...
void RSA2048::FF_skpow(BIG r[],BIG x[],BIG e[],BIG p[],int n)
{
    ...
    for (i=8*MODBYTES_B1024_58*n-1; i>=0; i--)
    {
        b=BIG_bit(e[i/BIGBITS_B1024_58],i%BIGBITS_B1024_58);
...
void RSA2048::FF_skspow(BIG r[],BIG x[],BIG e,BIG p[],int n)
{
    ...
    FF_nres(R0,p,n);
    FF_nres(R1,p,n);
    for (i=8*MODBYTES_B1024_58-1; i>=0; i--)
    {
        b=BIG_bit(e,i);
        FF_modmul(r,R0,R1,p,ND,n);
        FF_cswap(R0,R1,b,n);
...
#if CHUNK<32
    g=x[0][0]+((sign32)(x[0][1])<<BASEBITS_B1024_58);
#else
    g=(sign32)x[0][0];
#endif
    r=igcd(s,g);
