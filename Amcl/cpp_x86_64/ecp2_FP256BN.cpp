...
/* Set P=Q */
/* SU= 16 */
void FP256BN::ECP2_copy(ECP2 *P,ECP2 *Q)
{
//    P->inf=Q->inf;
    FP2_copy(&(P->x),&(Q->x));
    FP2_copy(&(P->y),&(Q->y));
...
static void ECP2_cmove(FP256BN::ECP2 *P,FP256BN::ECP2 *Q,int d)
{
    FP2_cmove(&(P->x),&(Q->x),d);
    FP2_cmove(&(P->y),&(Q->y),d);
    FP2_cmove(&(P->z),&(Q->z),d);
...
static void ECP2_select(FP256BN::ECP2 *P,FP256BN::ECP2 W[],sign32 b)
{
    FP256BN::ECP2 MP;
    sign32 m=b>>31;
    sign32 babs=(b^m)-m;
...
/* SU= 312 */
int FP256BN::ECP2_equals(ECP2 *P,ECP2 *Q)
{
    FP2 a,b;
...
/* SU= 232 */
void FP256BN::ECP2_affine(ECP2 *P)
{
    FP2 one,iz;
...
/* SU= 16 */
int FP256BN::ECP2_get(FP2 *x,FP2 *y,ECP2 *P)
{
	ECP2 W;
	ECP2_copy(&W,P);
	ECP2_affine(&W);
...
/* SU= 152 */
/* Output point P */
void FP256BN::ECP2_output(ECP2 *P)
{
    FP2 x,y;
    if (ECP2_isinf(P))
    {
...
/* SU= 232 */
void FP256BN::ECP2_outputxyz(ECP2 *P)
{
    ECP2 Q;
    if (ECP2_isinf(P))
    {
...
void FP256BN::ECP2_toOctet(octet *W,ECP2 *Q)
{
	BIG b;
	FP2 qx,qy;
    ECP2_get(&qx,&qy,Q);
    ...
    FP_redc(b,&(qx.b));
    BIG_toBytes(&(W->val[MODBYTES_B256_56]),b);
    FP_redc(b,&(qy.a));
    BIG_toBytes(&(W->val[2*MODBYTES_B256_56]),b);
    FP_redc(b,&(qy.b));
    BIG_toBytes(&(W->val[3*MODBYTES_B256_56]),b);

    W->len=4*MODBYTES_B256_56;

}

/* SU= 176 */
/* restore Q from octet string */
int FP256BN::ECP2_fromOctet(ECP2 *Q,octet *W)
{
	BIG b;
    FP2 qx,qy;
    BIG_fromBytes(b,&(W->val[0]));
	FP_nres(&(qx.a),b);
    BIG_fromBytes(b,&(W->val[MODBYTES_B256_56]));
    FP_nres(&(qx.b),b);
    BIG_fromBytes(b,&(W->val[2*MODBYTES_B256_56]));
    FP_nres(&(qy.a),b);
    BIG_fromBytes(b,&(W->val[3*MODBYTES_B256_56]));
    FP_nres(&(qy.b),b);
...
void FP256BN::ECP2_rhs(FP2 *rhs,FP2 *x)
{
...
/* SU= 232 */
int FP256BN::ECP2_set(ECP2 *P,FP2 *x,FP2 *y)
{
    FP2 rhs,y2;
...
/* SU= 232 */
int FP256BN::ECP2_setx(ECP2 *P,FP2 *x)
{
    FP2 y;
    ECP2_rhs(&y,x);
...
/* Set P+=Q */
/* SU= 400 */
int FP256BN::ECP2_add(ECP2 *P,ECP2 *Q)
{
...
/* Set P-=Q */
/* SU= 16 */
void FP256BN::ECP2_sub(ECP2 *P,ECP2 *Q)
{
	ECP2 NQ;
	ECP2_copy(&NQ,Q);
	ECP2_neg(&NQ);
...
/* P*=e */
/* SU= 280 */
void FP256BN::ECP2_mul(ECP2 *P,BIG e)
{
    /* fixed size windows */
    int i,nb,s,ns;
    BIG mt,t;
...
/* SU= 96 */
void FP256BN::ECP2_frob(ECP2 *P,FP2 *X)
{
    FP2 X2;
    FP2_sqr(&X2,X);
//printf("Into frob  %d\n",(P->z).b.XES);
...
//printf("x\n");
    FP2_conj(&(P->y),&(P->y));
//printf("za = %d zb= %d\n",(P->z).a.g[MPV_B256_56],(P->z).b.g[MPV_B256_56]);
    FP2_conj(&(P->z),&(P->z));
//printf("Into frob  %d\n",(P->z).b.XES);
    FP2_reduce(&(P->z));
    FP2_mul(&(P->x),&X2,&(P->x));
...
void FP256BN::ECP2_mul4(ECP2 *P,ECP2 Q[4],BIG u[4])
{
    int i,j,k,nb,pb,bt;
	ECP2 T[8],W;
    BIG t[4],mt;
	sign8 w[NLEN_B256_56*BASEBITS_B256_56+1];
	sign8 s[NLEN_B256_56*BASEBITS_B256_56+1];
...
void FP256BN::ECP2_mapit(ECP2 *Q,octet *W)
{
    BIG q,one,x,hv;
	FP Fx,Fy;
    FP2 X;
...
void FP256BN::ECP2_generator(ECP2 *G)
{
	FP2 wx,wy;
