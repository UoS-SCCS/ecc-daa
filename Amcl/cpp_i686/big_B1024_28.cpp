...
void B1024_28::BIG_mul(DBIG c,BIG a,BIG b)
{
    ...
    for (k=NLEN_B1024_28; k<2*NLEN_B1024_28-1; k++)
    {
        s-=d[k-NLEN_B1024_28];
        t=co+s;
        for (i=NLEN_B1024_28-1; i>=1+k/2; i--) t+=(dchunk)(a[i]-a[k-i])*(b[k-i]-b[i]);
        c[k]=(chunk)t&BMASK_B1024_28;
        co=t>>BASEBITS_B1024_28;
    }
    c[2*NLEN_B1024_28-1]=(chunk)co;
...
void B1024_28::BIG_monty(BIG a,BIG md,chunk MC,DBIG d)
{
    ...
    {
        t=c+s;
        for (i=NLEN_B1024_28-1; i>=1+k/2; i--) t+=(dchunk)(v[k-i]-v[i])*(md[i]-md[k-i]);
        a[k-NLEN_B1024_28]=(chunk)t&BMASK_B1024_28;
        c=(t>>BASEBITS_B1024_28)+d[k+1];
        s-=dd[k-NLEN_B1024_28+1];
    }
    a[NLEN_B1024_28-1]=(chunk)c&BMASK_B1024_28;
