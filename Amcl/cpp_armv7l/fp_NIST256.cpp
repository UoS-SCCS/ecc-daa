...
/* So when multiplying two numbers, their product *must* be less than MODBITS_NIST256+BASEBITS_B256_28*NLEN_B256_28 */
...
void NIST256::FP_mod(BIG r,DBIG d)
{
    ...
    if (MConst < NEXCESS_B256_28)
    {
		...
		BIG_norm(r);
        tw=r[NLEN_B256_28-1];
        r[NLEN_B256_28-1]&=TMASK_NIST256;
    ...
    else
    {
        ...
        r[NLEN_B256_28-1]&=TMASK_NIST256;
#if CHUNK == 16
        r[1]+=muladd(MConst,((tw>>TBITS_NIST256)+(v<<(BASEBITS_B256_28-TBITS_NIST256))),0,&r[0]);
#else
        r[0]+=MConst*((tw>>TBITS_NIST256)+(v<<(BASEBITS_B256_28-TBITS_NIST256)));
#endif
    }
    BIG_norm(r);
}
...
// Now multiply t by MConst..(?) 
    ...
    carry=r[NLEN_B256_28-1]>>TBITS_NIST256;
...
#endif
/*
    if (c<=NEXCESS_B256_28 && a->XES*c <= FEXCESS_NIST256)
	{
        BIG_imul(r->g,a->g,c);
		r->XES=a->XES*c;
		FP_norm(r);
...
/* SU= 160 */
void NIST256::FP_sqrt(FP *r,FP *a)
{
	FP v,i;
    BIG b;
    BIG m;
    ...
    if (MOD8_NIST256==3 || MOD8_NIST256==7)
    {
#if MODTYPE_NIST256 == PSEUDO_MERSENNE
		FP_fpow(r,a);
		FP_mul(r,r,a);
