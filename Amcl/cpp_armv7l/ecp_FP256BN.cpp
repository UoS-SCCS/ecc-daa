...
static void ECP_cswap(FP256BN::ECP *P,FP256BN::ECP *Q,int d)
{
    FP_cswap(&(P->x),&(Q->x),d);
#if CURVETYPE_FP256BN!=MONTGOMERY
    FP_cswap(&(P->y),&(Q->y),d);
...
static void ECP_cmove(FP256BN::ECP *P,FP256BN::ECP *Q,int d)
{
    FP_cmove(&(P->x),&(Q->x),d);
#if CURVETYPE_FP256BN!=MONTGOMERY
    FP_cmove(&(P->y),&(Q->y),d);
...
static void ECP_select(FP256BN::ECP *P,FP256BN::ECP W[],sign32 b)
{
    FP256BN::ECP MP;
    sign32 m=b>>31;
    sign32 babs=(b^m)-m;
...
/* Test P == Q */
/* SU=168 */
int FP256BN::ECP_equals(ECP *P,ECP *Q)
{
    FP a,b;
...
/* Set P=Q */
/* SU=16 */
void FP256BN::ECP_copy(ECP *P,ECP *Q)
{
//    P->inf=Q->inf;
    FP_copy(&(P->x),&(Q->x));
#if CURVETYPE_FP256BN!=MONTGOMERY
...
/* SU=56 */
void FP256BN::ECP_rhs(FP *v,FP *x)
{
#if CURVETYPE_FP256BN==WEIERSTRASS
    /* x^3+Ax+B */
    FP t;
...
int FP256BN::ECP_set(ECP *P,BIG x)
{
    BIG m,b;
	FP rhs;
    BIG_rcopy(m,Modulus);
...
int FP256BN::ECP_get(BIG x,ECP *P)
{
	ECP W;
	ECP_copy(&W,P);
	ECP_affine(W);
...
/* SU=16 */
int FP256BN::ECP_get(BIG x,BIG y,ECP *P)
{
    int s;
	ECP W;
	ECP_copy(&W,P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int FP256BN::ECP_set(ECP *P,BIG x,BIG y)
{
    FP rhs,y2;
...
/* SU=136 */
int FP256BN::ECP_setx(ECP *P,BIG x,int s)
{
    FP rhs;
	BIG t,m;
    BIG_rcopy(m,Modulus);
...
/* SU=120 */
void FP256BN::ECP_outputxyz(ECP *P)
{
    BIG x,z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void FP256BN::ECP_rawoutput(ECP *P)
{
	BIG x,y,z;
//   if (ECP_isinf(P))
 //   {
...
void FP256BN::ECP_toOctet(octet *W,ECP *P,bool compress)
{
#if CURVETYPE_FP256BN==MONTGOMERY
    BIG x;
    ECP_get(x,P);
...
int FP256BN::ECP_fromOctet(ECP *P,octet *W)
{
#if CURVETYPE_FP256BN==MONTGOMERY
    BIG x;
...
void FP256BN::ECP_add(ECP *P,ECP *Q,ECP *W)
{
	FP A,B,C,D,DA,CB;
...
/* Set P+=Q */
/* SU=248 */
void FP256BN::ECP_add(ECP *P,ECP *Q)
{
#if CURVETYPE_FP256BN==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  FP256BN::ECP_sub(ECP *P,ECP *Q)
{
	ECP NQ;
	ECP_copy(&NQ,Q);
	ECP_neg(&NQ);
...
void FP256BN::ECP_pinmul(ECP *P,int e,int bts)
{
    int i,b;
    ECP R0,R1;
...
/* Set P=r*P */
/* SU=424 */
void FP256BN::ECP_mul(ECP *P,BIG e)
{
#if CURVETYPE_FP256BN==MONTGOMERY
    /* Montgomery ladder */
    int nb,i,b;
...
void FP256BN::ECP_mul2(ECP *P,ECP *Q,BIG e,BIG f)
{
    BIG te,tf,mt;
    ECP S,T,W[8],C;
...
void FP256BN::ECP_mapit(ECP *P,octet *W)
{
    BIG q,x;
...
void FP256BN::ECP_generator(ECP *G)
{
	BIG x,y;
